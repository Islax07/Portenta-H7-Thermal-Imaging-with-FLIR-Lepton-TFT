...
#define ImageWidth 160 // Double the width for making the image in screen with double size 
#define ImageHeight 120 // Double the height
    ...
    for (int j = 0; j < LeptonHeight; j++) {
        for (int i = 0; i < LeptonWidth; i++) {
          uint16_t value=image[i][j];
            ...
            uint16_t color = colormap_RAINBOW_HC[scaled_value];
            tft.fillRect(xStart + (i * 2), yStart + (j * 2), 2, 2, color);
        }
    }
}
